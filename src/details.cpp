...
{
    auto tup = create_daily_file_sink_tuple(sinktb, trunct, std::move(base_filename), max_files);
...
auto gen_from_file_str(toml::string &&typestr, toml::table const &sinktb, bool const &trunct) -> spdlog::sink_ptr
{
...
auto gen_from_rotate_str(toml::string &&typestr, toml::table const &sinktb, toml::string &&base_filename,
                         std::size_t const &max_files) -> spdlog::sink_ptr
{
    if (typestr == "rotating_file_sink_st")
    {
        return create_rotating_file_sink_st_ptr(sinktb, std::move(base_filename), max_files);
    }
    if (typestr == "rotating_file_sink_st")
    {
        return create_rotating_file_sink_mt_ptr(sinktb, std::move(base_filename), max_files);
    }

    return nullptr;
}

auto gen_from_daily_str(toml::string &&typestr, toml::table const &sinktb, bool const &trunct,
                        toml::string &&base_filename, std::size_t const &max_files) -> spdlog::sink_ptr
{
    if (typestr == "daily_file_sink_st")
    {
        return create_daily_file_sink_st_ptr(sinktb, trunct, std::move(base_filename), max_files);
    }
    if (typestr == "daily_file_sink_mt")
    {
        return create_daily_file_sink_mt_ptr(sinktb, trunct, std::move(base_filename), max_files);
    }
...
#ifdef __linux__
auto gen_from_linux_str(toml::string &&typestr, toml::table const &sinktb) -> spdlog::sink_ptr
{
