...
void handle_multifiles(toml::string &&typestr, toml::table &&sinktb, spdlog::sink_ptr &sinkp, bool const &trunct)
{
    ...
    if (details::in_typelist(typestr, details::rotate_strs)) {
        sinkp = details::gen_from_rotate_str(std::move(typestr), std::move(sinktb), std::move(base_filename), max_files);
    }

    else if (details::in_typelist(typestr, details::daily_strs)) {
        sinkp = details::gen_from_daily_str(std::move(typestr), std::move(sinktb), trunct, std::move(base_filename), max_files);
    }
}
...
void handle_truncatable(toml::string &&typestr, toml::table &&sinktb, spdlog::sink_ptr &sinkp)
{
    ...
    if (details::in_typelist(typestr, details::file_strs)) {
        sinkp = details::gen_from_file_str(std::move(typestr), std::move(sinktb), trunct);
    } else if (details::in_typelist(typestr, details::rotate_strs) || details::in_typelist(typestr, details::daily_strs)) {
        handle_multifiles(std::move(typestr), std::move(sinktb), sinkp, trunct);
    }
}
...
void setup_sink(toml::table &&sinktb)
{
    ...
    if (sinktb.contains("level")) {
        level = sinktb.at("level").as_string();
    }
...
#ifdef __linux__
        sinkp = details::gen_from_linux_str(std::move(typestr), std::move(sinktb));
#else
        return;
#endif
